...
I will generate a key pair on my machine. Note that best practice is to include a pass-phrase to protect access to the private key however, for simplicity, we are not including this. 
...
```
Now I have 2 files `id_rsa.pem` and `id_rsa.pub.pem` I'll send the public key to you all in Slack. It's totally fine to share the public key. However I have to be guarded about the private key on my computer, but is fine as I don't need to move it or share it.
...
[This video](https://www.youtube.com/watch?v=hExRDVZHhig) explain how HTTPS uses a combination of asymmetric and symmetric encryption to secure communication between your browser and a web server. Can you locate the trusted certificate store on your machine? How could you use openssl to create a self-signed certifcate for testing?

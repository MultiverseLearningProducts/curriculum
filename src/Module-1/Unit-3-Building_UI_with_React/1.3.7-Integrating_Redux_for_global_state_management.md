...
1. The newly updated store triggers a re-render of all the components that depend on that _reactive_ variable with the newly updated value
...
From the top of the file, we are importing some tools and a __Provider__ Component from our newly added modules. Next we create a slice of a redux store, this has a name, state and reducers. We then create a redux store object by calling __configureStore__ and then pass this into the __Provider__ component. 
...
In the __ProductDetails__ component you can do the same. Create a selector that picks out a product from the products in the store i.e. __state.products.find__.

You can now remove the data import from the __App__ component, and remove the way we pass data down though the __Home__ component etc.
...
```
Notice on the last line above I'm exposing the __addToCart__ function so I can call it from else where in the app.

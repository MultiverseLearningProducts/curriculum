...
A lifecycle method is a method that fires when an event happens. They are consistent and predictable and in our case, the `mounted` method can be used to request data once the component has been mounted (added) to the DOM.
...
Part 1: Why is this error being thrown yet the image still loads?

...
Auth0 is used by [many well known companies](https://auth0.com/customers/) to secure their Web APIs. Auth0 can be used both for authenticaton of users and authorisation to access specific APIs. In this lesson we are going to focus on API authorisation via tokens issued by the Auth0 platform.

The diagram below illustrates the OAuth flow we are going to be using to secure our API resource. It consists of a client app requesting a token from Auth0, then passing this token as authorisation to access the Messages API:

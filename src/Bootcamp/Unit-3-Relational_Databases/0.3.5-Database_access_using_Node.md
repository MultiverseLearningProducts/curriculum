...
The code also makes use of [Prepared Statements](https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html) which are key for ensure your database applications are not vulnerable to [SQL injection attacks](https://portswigger.net/web-security/sql-injection). The "?" are substituated with the data hence your values have to be in the same order as the database columns. Watch this video to find out more about this kind of attack:

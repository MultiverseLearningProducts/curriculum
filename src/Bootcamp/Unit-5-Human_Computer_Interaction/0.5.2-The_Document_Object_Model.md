...
1. Recall the API methods on the `document` object that will enable programatic operations; create nodes, read values out of the DOM, update parts of the DOM tree, and delete nodes.
1. Implement a script that will render JSON structured data in the browser using functions on the `document` object.
...
```javascript
function async getRestaurantData() {
    const data = await fetch('http://localhost:3000/restaurants')
        .then(res => res.json())
        .catch(console.error)

    console.info(data) // now render this data in the DOM
}
...
* Use the boilerplate code above to fetch restaurant data from your API server and then write the code to render the data into the DOM.
* Execute your design (from previous previous session) as closely as you can.

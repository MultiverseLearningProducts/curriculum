...
function createTabHtml (title) {
    return `<a href="#${title.toLowerCase()}">${title}</a>`
}
function renderTabbedCodeBlockOpen (block) {
    return `<section><nav class="tabbed-code-block">${block.title.split('|').slice(1,-1).map(createTabHtml).join('')}</nav><article class="tabbed-code-block">`
}
